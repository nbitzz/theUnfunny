    ...
    return new Promise(async (resolve,reject) => {
            axios.get("https://unusann.us/api/v1/gets01metadata").then(async (data) => {
                resolve(JSON.stringify(data.data.map((v,x) => {
                    ...
                    return {name:v.title, image: `https://unusann.us/_next/image?url=https%3A%2F%2Fstream.unusann.us%2Fthumbnails%2F01%2F${`000`.slice(-(3-(x+1).toString().length))}${(x+1)}.webp&w=1920&q=100` }
                })))
            }).catch(reject)
    })
}